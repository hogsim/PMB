...
// +-------------------------------------------------+
// $Id: categ_see.inc.php,v 1.81.2.10 2015-03-13 13:28:47 mbertin Exp $
...
if ($id) {
	...
	if (mysql_num_rows($found_see_too)) {
		...
		while (($mesCategories_see_too = mysql_fetch_object($found_see_too))) {
			...
			if (category::has_notices($mesCategories_see_too->num_noeud)) $see_also.=  " <img src='$base_path/images/folder_search.gif' border=0 align='absmiddle'>";	
			else $see_also.=   " <img src='./images/folder.gif' border='0' align='middle'>";	
	...
	if ($nbr_lignes) {
		...
		if ($main) {
			...
			//Etendre
			if ($opac_allow_external_search) print "&nbsp;&nbsp;<span class=\"search_bt_external\"><a href='$base_path/index.php?search_type_asked=external_search&mode_aff=aff_module&external_type=simple'>".$msg["connecteurs_external_search_sources"]."</a></span>";
			//fin etendre			
		} else {
			//enregistrement de l'endroit actuel dans la session
			...
			//Etendre
			if ($opac_allow_external_search) print "&nbsp;&nbsp;<span class=\"search_bt_external\"><a href='$base_path/index.php?search_type_asked=external_search&mode_aff=aff_simple_search'>".$msg["connecteurs_external_search_sources"]."</a></span>";
