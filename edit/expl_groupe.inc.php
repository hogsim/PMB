...
// +-------------------------------------------------+
// $Id: expl_groupe.inc.php,v 1.35.6.3 2015-06-27 08:23:47 jpermanne Exp $
...
mysql_query("CREATE TEMPORARY TABLE ".$tempTable." ".$sql,$dbh);

$sql = "SELECT * FROM ".$tempTable;

$req_nombre_lignes_pret = mysql_query($sql,$dbh);

$nombre_lignes_pret = mysql_num_rows($req_nombre_lignes_pret);
...
while ($data = mysql_fetch_array($req)) {
	...
	if ($id_groupe!=$ancien_groupe) {
		...
		$reqcount = mysql_query($sqlcount,$dbh) or die(mysql_error()."<br />".$sqlcount);
		$nbok=0;
		$nbretard=0;
