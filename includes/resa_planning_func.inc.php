...
// +-------------------------------------------------+
// $Id: resa_planning_func.inc.php,v 1.21.6.1 2015-04-02 09:20:32 dbellamy Exp $
...
function planning_list($idempr=0, $idnotice=0, $order="",$info_gestion=NO_INFO_GESTION) {
	...
	switch ($info_gestion) {
		case GESTION_INFO_GESTION:
			...
			$res = mysql_query("SELECT idlocation, location_libelle, count(*) as nb FROM docs_location join empr on empr_location=idlocation join resa_planning on resa_idempr = id_empr group by idlocation, location_libelle order by location_libelle ",$dbh);
	...
	$r = mysql_query($q,$dbh) or die("Erreur SQL !<br />".$q."<br />".mysql_error());
		...
		switch ($info_gestion) {
			...
			case LECTEUR_INFO_GESTION:
				...
				if ($pmb_transferts_actif=="1") {
					...
					$libloc = @mysql_result(mysql_query($rqt,$dbh),0);
