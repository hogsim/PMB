...
// +-------------------------------------------------+
// $Id: pmbesEmpr.class.php,v 1.2.4.6 2015-08-04 15:48:52 mbertin Exp $
...
class pmbesEmpr extends external_services_api_class {
	...
	function fetch_empr($empr_cb='', $empr_id='') {
		...
		$res = mysql_query($sql);
		if (!$res || !mysql_num_rows($res)) return $this->build_ok($result,"Lecteur inconnu.",false);
	...
	function delete_empr($empr_cb='', $empr_id='') {
		...
		$sql = "SELECT id_empr, empr_cb FROM empr WHERE $where";
		$res = mysql_query($sql);
		if (!$res || !mysql_num_rows($res)) return $this->build_error( "Lecteur inconnu.");
	...
	function create_empr($empr_cb='',$fields) {
		...
		$res = mysql_query($sql);
		if (mysql_num_rows($res)) return $this->build_ok($result,"Le code $empr_cb est deja utilise.",false);
		...
		$r = mysql_query($q, $dbh);
		if (!mysql_num_rows($r)) return $this->build_ok($result,"Le champ 'statut' = ".$fields['statut']." n'est pas un Id present dans la base de donnee.",false);
		
		$q="select idcode from empr_codestat where idcode='".$fields['cstat']."' limit 1";
		$r = mysql_query($q, $dbh);
		if (!mysql_num_rows($r)) return $this->build_ok($result,"Le champ 'cstat' = ".$fields['cstat']." n'est pas un Id present dans la base de donnee.",false);
		
		$q="select id_categ_empr from empr_categ where id_categ_empr='".$fields['categ']."' limit 1";
		$r = mysql_query($q, $dbh);
		if (!mysql_num_rows($r)) return $this->build_ok($result,"Le champ 'categ' = ".$fields['categ']." n'est pas un Id present dans la base de donnee.",false);
	...
	function update_empr($empr_cb='', $empr_id=0, $fields) {
		...
		$sql = "SELECT id_empr, empr_cb FROM empr WHERE $where";
		$res = mysql_query($sql);
		if (!$res || !mysql_num_rows($res)) return $this->build_error( "Lecteur inconnu: 'empr_cb' = $empr_cb ou id_empr = $empr_id .");
		...
		$q="select idstatut from empr_statut where idstatut='".$fields['statut']."' limit 1";
		$r = mysql_query($q, $dbh);
		if (!mysql_num_rows($r)) return $this->build_error( "Le champ 'statut' = ".$fields['statut']." n'est pas un Id present dans la base de donnee.");
	
		$q="select idcode from empr_codestat where idcode='".$fields['cstat']."' limit 1";
		$r = mysql_query($q, $dbh);
		if (!mysql_num_rows($r)) return $this->build_error( "Le champ 'cstat' = ".$fields['cstat']." n'est pas un Id present dans la base de donnee.");
	
		$q="select id_categ_empr from empr_categ where id_categ_empr='".$fields['categ']."' limit 1";
		$r = mysql_query($q, $dbh);
		if (!mysql_num_rows($r)) return $this->build_error( "Le champ 'categ' = ".$fields['categ']." n'est pas un Id present dans la base de donnee.");
	...
	function add_in_caddie($empr_cb='', $empr_id=0, $caddie_id) {
		...
		$res = mysql_query($sql);
		if (!$res|| !mysql_num_rows($res) ) return $this->build_error( "Panier inconnu: 'caddie_id' = $caddie_id .");
	...
	function is_in_caddie($empr_cb='', $empr_id=0, $caddie_id) {
		...
		$sql = "SELECT id_empr, empr_cb FROM empr WHERE $where";
		$res = mysql_query($sql);
		if (!$res || !mysql_num_rows($res)) return $this->build_ok($result,"Lecteur inconnu: 'empr_cb' = $empr_cb ou id_empr = $empr_id",false);
		...
		$sql = "SELECT idemprcaddie FROM empr_caddie WHERE idemprcaddie = $caddie_id";
		$res = mysql_query($sql);
		if (!$res || !mysql_num_rows($res)) return $this->build_ok($result,"Panier inconnu: 'caddie_id' = $caddie_id .",false);
